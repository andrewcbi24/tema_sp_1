...
for i=1:length(t) %parcurgem vectorul de la 0 la 9
    if t(i)<=j*0.25 % verificam daca durata unui nivel este <= 0.25s
        if j<=length(t)
...
s=s*200-7  %am observat ca semnanului intial trebuia inmultit cu 200 pentru a atinge nivelurile de 1,3,5,7 apoi am scazut 7 pentru a obtine niveluri de la -7 la 7
